...
def extract_from_csv(infile, outfile):
    df = pd.read_csv(infile, header=None, names=csv_columns)[
...
def extract_from_tsv(infile, outfile):
    df = pd.read_csv(infile, header=None, names=tsv_columns, delimiter="\t")[
...
(
    download_data
    >> unzip_data
    >> [extract_from_CSV, extract_from_TSV, extract_from_fixed_width_file]
    >> consolidate_data
    >> transform_data
    >> create_table
)
